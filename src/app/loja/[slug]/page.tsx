...
type Store = { id:string; name:string; description:string|null; hero_title:string|null; subtitle:string|null; show_hero_content:boolean; logo_path:string|null; banner_path:string|null; whatsapp:string; theme_primary:string; theme_accent:string; theme_background:string; theme_text:string };
...
export default async function PublicStorePage({ params }: { params:Promise<{slug:string}> }) {
  ...
  const {data:storeData}=await supabase.from("stores").select("id, name, description, hero_title, subtitle, show_hero_content, logo_path, banner_path, whatsapp, theme_primary, theme_accent, theme_background, theme_text").eq("slug",slug).eq("is_active",true).maybeSingle();
  ...
  const [{data:productsData},{data:notificationSettings}]=await Promise.all([supabase.from("products").select("id, name, description, price_cents, sale_price_cents, track_stock, stock_quantity, categories(name), product_images(storage_path, position)").eq("store_id",store.id).eq("is_active",true).order("created_at",{ascending:false}),supabase.from("store_notification_settings").select("promotions_enabled").eq("store_id",store.id).maybeSingle()]);
