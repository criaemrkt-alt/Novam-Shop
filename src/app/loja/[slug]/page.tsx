...
type Store = { id:string; name:string; description:string|null; hero_title:string|null; subtitle:string|null; logo_path:string|null; banner_path:string|null; whatsapp:string; theme_primary:string; theme_accent:string; theme_background:string; theme_text:string };
...
export default async function PublicStorePage({ params }: { params:Promise<{slug:string}> }) {
  ...
  const {data:storeData}=await supabase.from("stores").select("id, name, description, hero_title, subtitle, logo_path, banner_path, whatsapp, theme_primary, theme_accent, theme_background, theme_text").eq("slug",slug).eq("is_active",true).maybeSingle();
