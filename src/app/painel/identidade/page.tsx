...
export default async function BrandingPage({ searchParams }: { searchParams: Promise<{ erro?: string; sucesso?: string }> }) {
  ...
  const { data: store } = await supabase.from("stores").select("name, hero_title, subtitle, logo_path, banner_path, theme_preset, theme_primary, theme_accent, theme_background, theme_text").eq("owner_id", user.id).maybeSingle();
