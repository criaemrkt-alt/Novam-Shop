...
import { ProductVariationsEditor } from "@/components/product-variations-editor";

type Product = { id:string; name:string; display_position:number; price_cents:number; sale_price_cents:number|null; is_active:boolean; track_stock:boolean; stock_mode:"product"|"variant"; stock_quantity:number|null; categories:{ name:string }|null; product_images:{ storage_path:string }[]; product_variants:{stock_quantity:number|null;is_active:boolean}[] };
...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ erro?: string; sucesso?: string }> }) {
  ...
  const [{ data: categories }, { data: productsData }] = await Promise.all([
    ...
    supabase.from("products").select("id, name, display_position, price_cents, sale_price_cents, is_active, track_stock, stock_mode, stock_quantity, categories(name), product_images(storage_path), product_variants(stock_quantity, is_active)").eq("store_id", store.id).order("display_position").order("created_at", { ascending:false }),
  ]);
      ...
      <div className="catalog-main">
        <details className="product-create" open={products.length === 0}><summary><span>Adicionar produto</span><b>+</b></summary>
          <form action="/api/products" method="post" encType="multipart/form-data" className="feature-form product-form">
            ...
            <details className="product-extra-fields"><summary>Adicionar materiais, prazo e personalização <b>+</b></summary><div><div className="form-row"><label className="field"><span>Materiais e acabamento</span><textarea name="materials" rows={3} maxLength={500}/></label><label className="field"><span>Prazo</span><textarea name="lead_time" rows={3} maxLength={300}/></label></div><label className="field"><span>Personalização e observações</span><textarea name="customization_notes" rows={3} maxLength={1000}/></label></div></details>
        ...
        <div className="product-list">{products.length === 0 ? <div className="empty-state"><span>01</span><h2>Seu primeiro produto começa aqui.</h2><p>Cadastre uma peça para começar a construir sua vitrine.</p></div> : products.map((product,index) => <article key={product.id}>
          ...
          <div className="product-list-copy"><span>{product.categories?.name || "Sem categoria"}</span><Link href={`/painel/produtos/${product.id}`}>{product.name}</Link><small>{product.stock_mode==="variant"?`${product.product_variants.length} combinações · ${product.track_stock?`${product.product_variants.reduce((sum,variant)=>sum+(variant.stock_quantity??0),0)} em estoque`:"estoque ilimitado"}`:product.track_stock ? `${product.stock_quantity ?? 0} em estoque` : "Estoque ilimitado"}</small></div>
          ...
          <div className="product-order-actions" aria-label={`Posição de ${product.name}`}><span>POSIÇÃO {String(index+1).padStart(2,"0")}</span><form action="/api/products/order" method="post"><input type="hidden" name="id" value={product.id}/><input type="hidden" name="direction" value="up"/><button type="submit" disabled={index===0} aria-label={`Subir ${product.name}`}>↑</button></form><form action="/api/products/order" method="post"><input type="hidden" name="id" value={product.id}/><input type="hidden" name="direction" value="down"/><button type="submit" disabled={index===products.length-1} aria-label={`Descer ${product.name}`}>↓</button></form></div>
        </article>)}</div>
      </div>
