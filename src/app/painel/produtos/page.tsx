...
type Product = { id:string; name:string; display_position:number; price_cents:number; sale_price_cents:number|null; is_active:boolean; track_stock:boolean; stock_quantity:number|null; categories:{ name:string }|null; product_images:{ storage_path:string }[] };
...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ erro?: string; sucesso?: string }> }) {
  ...
  const [{ data: categories }, { data: productsData }] = await Promise.all([
    ...
    supabase.from("products").select("id, name, display_position, price_cents, sale_price_cents, is_active, track_stock, stock_quantity, categories(name), product_images(storage_path)").eq("store_id", store.id).order("display_position").order("created_at", { ascending:false }),
  ]);
