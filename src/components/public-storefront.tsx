...
export function PublicStorefront({store,products}:{store:Store;products:Product[]}) {
  ...
  return <>
    ...
    <section id="produtos" className="public-products"><div className="public-products-heading"><div><span>CATÁLOGO · {activeCategory.toUpperCase()}</span><h2>{activeCategory==="Todos"?"Descubra nossos produtos":activeCategory}</h2></div><p>{visibleProducts.length} {visibleProducts.length===1?"produto":"produtos"}</p></div>
      {visibleProducts.length?<div className="public-product-grid">{visibleProducts.map(product=>{const quantity=cart[product.id]??0;const unavailable=product.track_stock&&(product.stock_quantity??0)<=0;const atLimit=product.track_stock&&quantity>=(product.stock_quantity??0);return <article key={product.id}><div className="public-product-photo"><Link href={`/loja/${store.slug}/produto/${product.id}`} aria-label={`Ver ${product.name}`}>{product.image_url?<Image src={product.image_url} alt={product.name} fill sizes="(max-width:700px) 50vw, 25vw"/>:<span>{product.name.slice(0,1)}</span>}</Link>{unavailable?<span className="catalog-unavailable">Indisponível</span>:quantity>0?<div className="catalog-quantity" aria-label={`${quantity} unidades na sacola`}><button type="button" onClick={()=>changeQuantity(product.id,-1)} aria-label={`Remover uma unidade de ${product.name}`}>−</button><strong>{quantity}</strong><button type="button" onClick={()=>changeQuantity(product.id,1)} disabled={atLimit} aria-label={`Adicionar mais uma unidade de ${product.name}`}>+</button></div>:<button type="button" onClick={()=>toggle(product.id)}>Adicionar à sacola</button>}</div><small>{product.category||"Novidades"}</small><h3><Link href={`/loja/${store.slug}/produto/${product.id}`}>{product.name}</Link></h3><div>{product.sale_price_cents!==null&&<del>{money(product.price_cents)}</del>}<strong>{money(product.sale_price_cents??product.price_cents)}</strong></div></article>})}</div>:<div className="public-empty"><h2>Nenhum produto nesta categoria.</h2><p>Escolha outra categoria para continuar navegando.</p></div>}
    </section>
