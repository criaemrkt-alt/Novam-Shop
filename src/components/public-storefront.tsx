...
type Store = { id:string; slug:string; name:string; description:string|null; logo_url:string|null; banner_url:string|null; whatsapp:string; promotions_enabled:boolean };
...
export function PublicStorefront({store,products}:{store:Store;products:Product[]}) {
  ...
  return <>
    ...
    <section id="produtos" className="public-products"><div className="public-products-heading"><div><span>CATÁLOGO · {activeCategory.toUpperCase()}</span><h2>{activeCategory==="Todos"?"Descubra nossos produtos":activeCategory}</h2></div><p>{visibleProducts.length} {visibleProducts.length===1?"produto":"produtos"}</p></div>
      {visibleProducts.length?<div className="public-product-grid">{visibleProducts.map(product=><article key={product.id}><div className="public-product-photo"><Link href={`/loja/${store.slug}/produto/${product.id}`} aria-label={`Ver ${product.name}`}>{product.image_url?<Image src={product.image_url} alt={product.name} fill sizes="(max-width:700px) 50vw, 25vw"/>:<span>{product.name.slice(0,1)}</span>}</Link><button className={cart[product.id]?"in-cart":""} type="button" onClick={()=>toggle(product.id)}>{cart[product.id]?"Remover da sacola":"Adicionar à sacola"}</button></div><small>{product.category||"Novidades"}</small><h3><Link href={`/loja/${store.slug}/produto/${product.id}`}>{product.name}</Link></h3><div>{product.sale_price_cents!==null&&<del>{money(product.price_cents)}</del>}<strong>{money(product.sale_price_cents??product.price_cents)}</strong></div></article>)}</div>:<div className="public-empty"><h2>Nenhum produto nesta categoria.</h2><p>Escolha outra categoria para continuar navegando.</p></div>}
    </section>
    ...
    <div className={cartOpen?"cart-backdrop visible":"cart-backdrop"} onClick={()=>setCartOpen(false)}/><aside className={cartOpen?"shop-cart-drawer open":"shop-cart-drawer"} aria-hidden={!cartOpen}><div className="cart-drawer-head"><div><span>SUA SACOLA</span><strong>{count} {count===1?"item":"itens"}</strong></div><button type="button" onClick={()=>setCartOpen(false)} aria-label="Fechar sacola">×</button></div>
        <div className="cart-drawer-items">{cartItems.length?cartItems.map(item=><article key={item.id}>{item.image_url?<div className="cart-drawer-photo"><Image src={item.image_url} alt="" fill sizes="76px"/></div>:<div className="cart-drawer-photo empty">{item.name.slice(0,1)}</div>}<div><small>{item.category||"Produto"}</small><strong>{item.name}</strong><span>{money(item.sale_price_cents??item.price_cents)}</span><div className="cart-quantity"><button type="button" onClick={()=>changeQuantity(item.id,-1)}>−</button><b>{item.quantity}</b><button type="button" onClick={()=>changeQuantity(item.id,1)}>+</button></div></div><button className="cart-remove" type="button" onClick={()=>remove(item.id)}>Remover</button></article>):<div className="cart-empty"><BagIcon/><h2>Sua sacola está vazia.</h2><p>Escolha um produto para começar seu pedido.</p><button type="button" onClick={()=>setCartOpen(false)}>Continuar comprando</button></div>}</div>
