...
type Product = { id:string; name:string; display_position:number; description:string|null; price_cents:number; sale_price_cents:number|null; track_stock:boolean; stock_quantity:number|null; categories:{name:string}|null; product_images:{storage_path:string;position:number}[] };
...
export default async function PublicStorePage({ params }: { params:Promise<{slug:string}> }) {
  ...
  const [{data:productsData},{data:notificationSettings},{data:bannersData}]=await Promise.all([supabase.from("products").select("id, name, display_position, description, price_cents, sale_price_cents, track_stock, stock_quantity, categories(name), product_images(storage_path, position)").eq("store_id",store.id).eq("is_active",true).order("display_position").order("created_at",{ascending:false}),supabase.from("store_notification_settings").select("promotions_enabled").eq("store_id",store.id).maybeSingle(),supabase.from("store_banners").select("desktop_path, mobile_path, position").eq("store_id",store.id).order("position")]);
