...
export function PublicStorefront({store,products}:{store:Store;products:Product[]}) {
  ...
  return <>
    ...
    <section className={`public-shop-hero${store.show_hero_content?"":" image-only"}`} onTouchStart={event=>{touchStartX.current=event.touches[0]?.clientX??null;}} onTouchEnd={event=>{if(touchStartX.current===null)return;const distance=(event.changedTouches[0]?.clientX??touchStartX.current)-touchStartX.current;if(Math.abs(distance)>45)moveBanner(distance>0?-1:1);touchStartX.current=null;}}>{store.banners.length>0&&<div className="shop-hero-media">{store.banners.map((banner,index)=><picture key={banner.desktop_url} className={index===activeBanner?"active":""}>{banner.mobile_url&&<source media="(max-width: 700px)" srcSet={banner.mobile_url}/>}<Image src={banner.desktop_url} alt="" fill priority={index===0} sizes="100vw"/></picture>)}</div>}{store.show_hero_content&&<div className="shop-hero-copy"><span>LOJA OFICIAL</span><h1>{store.hero_title||`${store.name}, do seu jeito.`}</h1><a href="#produtos">Ver coleção</a></div>}{store.banners.length>1&&<div className="shop-hero-controls"><button type="button" onClick={()=>moveBanner(-1)} aria-label="Banner anterior">←</button><div>{store.banners.map((_,index)=><button key={index} type="button" className={index===activeBanner?"active":""} onClick={()=>setActiveBanner(index)} aria-label={`Ver banner ${index+1}`}/>)}</div><button type="button" onClick={()=>moveBanner(1)} aria-label="Próximo banner">→</button></div>}</section>
